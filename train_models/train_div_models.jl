...
for I in Is
    ...
    function div_model(input;mesh = coarse_mesh,NN = model.eval,SO = SO)

        tau = NN(input)
        return divergence_model(tau,mesh,SO)
    end

    NN_rhs(u_bar,mesh,t;setup= setup,rhs = coarse_rhs,Re = Re,model = div_model,B =model.B,other_arguments = 0) = neural_rhs(u_bar,mesh,t;setup= setup,rhs = rhs,Re = Re,model = model,B = B)
