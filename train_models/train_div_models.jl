...
for I in Is
    ...
    function div_model(input;mesh = mesh,NN = model.eval,SO = SO)

        tau = NN(input)
        return divergence_model(tau,mesh,SO)
    end

    NN_rhs(u_bar,mesh,t;setup= setup,rhs = coarse_rhs,Re = Re,model = div_model,B =B,other_arguments = 0) = neural_rhs(u_bar,mesh,t;setup= setup,rhs = rhs,Re = Re,model = model,B = B)
    ...
    savefig("div_loss_"*"$I"*".png")
    plot(relative_losses,marker = true)
    savefig("div_relative_"*"$I"*".png")
    

    save_NN_model(model,"./models/div_"*"$I"*"/div_model")
    save("./models/div_"*"$I"*"/losses.jld","losses",losses)
    save("./models/div_"*"$I"*"/relative_losses.jld","relative_losses",relative_losses)
end