...
function gen_pressure_solver(mesh,O)
    ...
    if use_GPU
        pad_test = cu(padding(test,Tuple((1 for i in 1:dims)),circular = true))
    else
    ...
    if use_GPU
        lambda[[(1:1) for i in 1:dims]...] .= cu(1)
        zero_mask = cu(ones(N))

        zero_mask[[1:1 for i in 1:dims]...] .= cu(0)
    else
        lambda[[(1:1) for i in 1:dims]...] .= 1
        zero_mask = ones(N)
        zero_mask[[1:1 for i in 1:dims]...] .= 0
    end
...
function gen_rhs(setup,F;Re = 1000,damping = 0)
    ...
    function rhs(V,mesh,t;Re = Re,O=setup.O,PS = setup.PS,F = Float32.(padding(F,(1,1),circular = true)),solve_pressure = true,damping = damping,other_arguments = 0)
        ...
        DV = T(1/Re) .* O.D(pad_V[[(2:size(pad_V)[i]-1) for i in 1:dims]...,:,:])
        ...
        if damping != 0
            rhs -= T(damping)*pad_V[[(3:size(pad_V)[i]-2) for i in 1:dims]...,:,:]
        end
        ...
        if solve_pressure
            r = O.M(rhs)
            p = PS(r)
            return rhs[[(2:size(rhs)[i]-1) for i in 1:dims]...,:,:] - O.G(padding(p,Tuple((1 for i in 1:dims)),circular = true))
        else
            return rhs[[(2:size(rhs)[i]-1) for i in 1:dims]...,:,:]
        end
