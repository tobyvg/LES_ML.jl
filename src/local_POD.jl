...
function add_filter_to_modes(POD_modes,MP;orthogonalize = false)
    ...
    for i in 2:r
        ...
        norm_i =  sum(MP.one_reconstructor(T(1/(prod(MP.fine_mesh.N)))*MP.one_filter(mode_i .* mode_i)),dims = collect(1:dims+1))
...
function gen_projection_operators(POD_modes,MP;uniform = false)
    ...
    #print(typeof(MP.omega_tilde))
    sqrt_omega_tilde = sqrt.(MP.omega_tilde)
    #print(typeof(sqrt_omega_tilde))
    inv_sqrt_omega_tilde = 1 ./ sqrt_omega_tilde
    #print(typeof(inv_sqrt_omega_tilde))
