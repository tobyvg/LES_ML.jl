...
function cons_mom_B(B_kernel;channels = 1,use_GPU = false)
    if B_kernel != 0
        for channel in channels
            dims = length(size(B_kernel))-2
            channel_mask = gen_channel_mask(B_kernel,channel,use_GPU = use_GPU)
...
function find_padding_size(CNN,test_size = 100; use_GPU = false)
...
function gen_channel_mask(data,channel;use_GPU = false)
    ...
    stop_gradient() do
        channel_mask[[(:) for i in 1:dims]...,channel:channel] .+= 1
    end
    return channel_mask
end
...
function gen_NN(kernel_sizes,channels,strides,r,B;UPC = 0,boundary_padding = 0,constrain_energy = true,conserve_momentum=true,dissipation = true,use_GPU = false)
    ...
    function NN(input;a = 0,CNN = CNN,r = r,B = [B...],B_mats = B_mats,UPC = UPC,pad_size = pad_size,boundary_padding = boundary_padding,constrain_energy =constrain_energy,conserve_momentum = conserve_momentum,dissipation = dissipation,use_GPU = use_GPU)
        ...
        if constrain_energy
            ...
            if conserve_momentum
                ...
                B3 = cons_mom_B(B3,channels = collect(1:UPC),use_GPU = use_GPU)
            end
            ...
            if dissipation
                c_tilde -=  Flux.conv(psi.^2 .* Flux.conv(a,B3),B3_T)
            end
...
function load_NN_model(name;use_GPU = false)
    ...
    if use_GPU
        for i in 1:length(model.CNN)
            ...
            model.CNN[i].bias .= cu(CNN_weights_and_biases[i][2])
        end

        for i in 1:length(model.B_mats)
            model.B_mats[i] = cu(B_mats[i])
        end
    else
        for i in 1:length(model.CNN)
